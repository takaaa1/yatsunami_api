...
export class AuthService {
  ...
  private async notifyAdminsNewUser(user: {
    id: string;
    nome: string;
    email: string;
  }) {
  ...
  private async verifyAndMigratePassword(
    ...
    try {
      const rows: Array<{ encrypted_password: string }> = await this.prisma
        .$queryRaw`
  ...
  async register(registerDto: RegisterDto): Promise<AuthResponseDto> {
    ...
    void this.notifyAdminsNewUser({
      id: user.id,
      nome: user.nome,
      email: user.email,
    }).catch((error) =>
      this.logger.error(`Falha ao notificar admins do novo usuário: ${error}`),
    );
  ...
  async updateProfile(
    ...
    if (file) {
      ...
      const fileName = imageUploadFileName(
        `${userId}_${Date.now()}`,
        contentType,
      );
      ...
      updateData.avatarUrl = this.storageService.getPublicUrl(
        'avatars',
        fileName,
      );
      ...
      if (currentUser?.avatarUrl) {
        try {
          ...
          this.logger.warn(`Failed to delete old avatar: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    }
  ...
  async forgotPassword(
    ...
    const expirationMinutes = this.configService.get<number>(
      'auth.resetPasswordExpirationMinutes',
      15,
    );
    ...
    await this.prisma.codigoRecuperacao.deleteMany({
      where: {
        OR: [{ email: emailLower }, { expiraEm: { lt: new Date() } }],
      },
    });
  ...
  async deleteOwnAccount(userId: string): Promise<{ message: string }> {
    ...
    if (user.avatarUrl) {
      try {
        const avatarPath = this.storageService.extractPathFromUrl(
          user.avatarUrl,
          'avatars',
        );
        if (avatarPath) {
        ...
        this.logger.warn(
          `Failed to delete avatar for user ${userId}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
