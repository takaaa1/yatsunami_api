...
describe('DeliveryController — updateLocation', () => {
    const payload = { formId: 10, courierId: 1, latitude: -23.5, longitude: -46.6, userId: 'user-1' } as any;
    ...
    beforeEach(() => {
        emit = jest.fn();
        deliveryService = {
            updateLocation: jest.fn().mockResolvedValue({ id: 1, reactivatedOrderIds: [] }),
        };
        trackingGateway = {
    ...
    it('avisa os clientes quando o rastreio é reativado automaticamente', async () => {
        deliveryService.updateLocation.mockResolvedValue({ id: 1, reactivatedOrderIds: [101, 102] });

        await controller.updateLocation(payload);

        expect(trackingGateway.broadcastSharingStatus).toHaveBeenCalledWith(10, true, {
            userId: 'user-1',
            courierId: 1,
        });
        expect(realtimeGateway.broadcast).toHaveBeenCalledWith('pedidos_encomenda', 'UPDATE', { id: 101, emEntrega: true });
        expect(realtimeGateway.broadcast).toHaveBeenCalledWith('pedidos_encomenda', 'UPDATE', { id: 102, emEntrega: true });
    });
