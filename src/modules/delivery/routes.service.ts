...
export class RoutesService {
  ...
  constructor(
    ...
    this.googleMapsKey = (
      this.configService.get<string>('googleMapsKey') || ''
    ).trim();
  ...
  async optimizeRoute(
    ...
    if (!destinations.length)
      return { orderedDestinations: [], arrivalTimes: [] };
    ...
    this.logger.debug(
      `Google Maps Request URL: ${url.replace(/key=[^&]+/, 'key=REDACTED')}`,
    );

    try {
      const { data } = await firstValueFrom(
        this.httpService.get<DirectionsResponse>(url),
      );
      ...
      if (data.status !== 'OK') {
        ...
        if (data.status === 'NOT_FOUND') {
          this.logger.error(
            'Check your origin and destination addresses. One or more could not be geocoded.',
          );
        }
      ...
      const optimizedOrder: number[] =
        waypoints.length === 0
          ...
          : Array.isArray(route.waypoint_order) &&
              route.waypoint_order.length === waypoints.length
      ...
      const inputDwellOk =
        Array.isArray(waypointDwellSeconds) &&
        waypointDwellSeconds.length === waypoints.length;
      const orderedDwellSeconds: number[] =
        waypoints.length === 0
          ? []
          : optimizedOrder.map((idx) => {
              const raw = inputDwellOk
                ? waypointDwellSeconds[idx]
                : defaultDwell;
              const n =
      ...
      const orderedDestinations =
        waypoints.length === 0
          ? []
          : optimizedOrder.map((index) => waypoints[index]);
  ...
  async arrivalsForFixedOrder(
    ...
    this.logger.debug(
      `Google Maps fixed-order URL: ${url.replace(/key=[^&]+/, 'key=REDACTED')}`,
    );
    ...
    if (data.status !== 'OK') {
      this.logger.error(
        `Google Maps API error (fixed order): ${data.status} - ${data.error_message || ''}`,
      );
      throw new Error(`Google Maps API error: ${data.status}`);
    }
    ...
    for (let j = 0; j < legs.length; j++) {
      ...
      if (j < legs.length - 1) {
        ...
        const n =
          typeof raw === 'number' && Number.isFinite(raw) ? raw : defaultDwell;
    ...
    const tryNominatim = async (query: string) => {
      ...
      const response = await firstValueFrom(
        this.httpService.get<NominatimResult[]>(url, {
          headers: { 'User-Agent': 'Yatsunami-App/1.0' },
        }),
      );
    ...
    try {
      ...
      if (cepFallback) {
        ...
        if (google2) {
          return {
            ...google2,
            formattedAddress: `${google2.formattedAddress} (via CEP ${cepFormatted})`,
          };
        }
        ...
        if (osm2) {
          return {
            ...osm2,
            formattedAddress: `${osm2.formattedAddress} (via CEP ${cepFormatted})`,
          };
        }
      }
      ...
      this.logger.error(
        `Geocoding error for "${address}": ${e instanceof Error ? e.message : String(e)}`,
      );
      return null;
    }
  }
