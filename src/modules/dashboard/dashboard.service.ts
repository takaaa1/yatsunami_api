...
function classifyProximoPedidoStatus(
  ...
  const startDate = form.dataInicioPedido
    ? new Date(form.dataInicioPedido)
    : null;
 ...
 */
function sortProximosPedidosForUser(
  items: ProximoPedidoDto[],
  now: Date,
  max: number,
): ProximoPedidoDto[] {
  ...
  openLike.sort(
    (a, b) =>
      deliveryDateStartOfDayTs(a.data_entrega) -
      deliveryDateStartOfDayTs(b.data_entrega),
  );
  closed.sort(
    (a, b) =>
      deliveryDateStartOfDayTs(b.data_entrega) -
...
export class DashboardService {
  ...
  async getAdminDashboard(filters?: { year?: number; month?: number }) {
    ...
    const month =
      filters?.month !== undefined ? filters.month : now.getMonth() + 1;
    ...
    const unifiedHistory = this.groupByMonthUnified(
      vendasHistory,
      despesasHistory,
      chartStart,
      endDate,
  ...
  async getUserDashboard(userId: string) {
    ...
    const formAberta = await this.prisma.dataEncomenda.findFirst({
      where: {
        ...
        OR: [{ dataInicioPedido: null }, { dataInicioPedido: { lte: now } }],
      },
    ...
    const proximosWhere = {
      OR: [{ ativo: true }, { dataLimitePedido: { gte: sevenDaysAgo } }],
    };
    ...
    const proximosMapped: ProximoPedidoDto[] = Array.from(
      mergedById.values(),
    ).map((form) => ({
      id: form.id,
      data_entrega:
    ...
    return {
      ...
      formAberta: formAberta
        ? {
            id: formAberta.id,
            data_entrega:
              formAberta.dataEntrega instanceof Date
                ? formAberta.dataEntrega.toISOString().split('T')[0]
                : formAberta.dataEntrega,
  ...
  private groupByMonthUnified(
    vendas: { data: Date; _sum: { total: Prisma.Decimal | null } }[],
    despesas: {
      dataCompra: Date | null;
      _sum: { valorTotal: Prisma.Decimal | null };
    }[],
    chartStart: Date,
    chartEnd: Date,
  ) {
    const result: Record<string, { revenue: number; expenses: number }> = {};
    ...
    despesas.forEach((item) => {
      // `dataCompra` é anulável: sem data não há mês para agregar.
      // (antes, `new Date(null)` virava 1970 e caía fora do range)
      if (!item.dataCompra) return;
  ...
  private groupByMonth(
    data: (Record<string, unknown> & { _sum: Record<string, unknown> })[],
    dateField: string,
    valueField: string,
  ) {
