...
import type { ItemNota, NfeDetalhe, NfeRoot } from './nfe.types';
...
export class QrParserService {
  ...
  private parseXml(xml: string, url: string) {
    const jsonObj = this.xmlParser.parse(xml) as NfeRoot;
    ...
    const itens: ItemNota[] = det.map((d: NfeDetalhe) => ({
    ...
    return {
      ...
      valorTotalSemDesconto: parseFloat(
        infNfe?.['total']?.['ICMSTot']?.['vProd'] || '0',
      ),
      valorDesconto: parseFloat(
        infNfe?.['total']?.['ICMSTot']?.['vDesc'] || '0',
      ),
      urlQrcode: url,
      itens,
      xmlRaw: xml,
  ...
  private parseHtml($: CheerioAPI, url: string) {
    const nomeEstabelecimento =
      $('.txtTopo').first().text().trim() || 'Desconhecido';

    let dataCompra = new Date().toISOString();
    const emissaoStrong = $('strong').filter(
      (_, el) =>
        !!$(el)
          .text()
          .match(/Emissão/i),
    ...
    if (emissaoStrong.length > 0) {
      ...
      const matchDateTime = text.match(
    ...
    const valorTotal =
      extrairValor('Valor a pagar') || extrairValor('Valor total');
    const valorDesconto = extrairValor('Descontos');
    const valorTotalSemDesconto =
      extrairValor('Total bruto') || extrairValor('Valor total') || valorTotal;
    ...
    $('table#tabResult tr').each((_, el) => {
      ...
      if (descricao) {
        const extractNum = (selector: string, labelRegex: RegExp) => {
          ...
          if (anyNum) {
            ...
            if (numStr.includes('.') && numStr.includes(',')) {
              return (
                parseFloat(numStr.replace(/\./g, '').replace(',', '.')) || 0
              );
            }
        ...
        const valor =
          parseFloat(vtotalText.replace(/\./g, '').replace(',', '.')) ||
