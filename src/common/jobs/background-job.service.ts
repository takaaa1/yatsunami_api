...
import { BackgroundJobRecord, BackgroundJobStatus } from './background-job.types';
import { CronLockService } from '../cron/cron-lock.service';
import { CRON_LOCK_KEYS, isBackgroundJobRuntime } from '../runtime/runtime.config';
...
export class BackgroundJobService {
    ...
    fireAndForget(name: string, fn: () => Promise<void>): void {
        if (!isBackgroundJobRuntime()) {
            void fn().catch((error) => {
                this.logger.error(`Background job "${name}" failed (sync fallback): ${error}`);
            });
            return;
        }
    ...
    getJob<TResult = unknown>(id: string): BackgroundJobRecord<TResult> | undefined {
        ...
        void this.cronLockService.withLock(
            ...
            () => {
                ...
                for (const [id, job] of this.jobs.entries()) {
                    const finishedAt = job.completedAt?.getTime() ?? job.createdAt.getTime();
